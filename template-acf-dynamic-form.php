...
<script>
    ...
    function renderNav(index) {
        ...
        if (index === steps.length) {
            ...
            buyBtn.type = 'button';
            buyBtn.className = 'btn btn-success ms-auto';
            buyBtn.textContent = 'Buy & Download';
            // 💡 Здесь можно добавить реальную оплату позже
    ...
    function showStep(index) {
        ...
        // Показываем/скрываем поля формы по шагам
        ...
        // Блок оплаты
        const paymentBlock = document.getElementById('payment-step');
        if (index === steps.length) {
            if (paymentBlock) paymentBlock.classList.remove('d-none');
        } else {
            if (paymentBlock) paymentBlock.classList.add('d-none');
        }
