...
function handle_generate_pdf() {
    ...
    if (!$acf_template_slug) {
        // fallback: пост slug
        $acf_template_slug = 'group_' . str_replace('-', '_', get_post_field('post_name', $post_id));
    }

    // ✅ Используем без добавления group_
    $template_file = sanitize_title($acf_template_slug);
    $template_path = get_template_directory() . "/acf-templates/{$template_file}.php";

    if (!file_exists($template_path)) {
        wp_die("❌ Шаблон PDF не найден: acf-templates/{$template_file}.php");
    }
...
add_action('acf/save_post', function($post_id) {
    ...
    // Получаем страницу, с которой отправлена форма (referer)
    $ref = wp_get_referer();
    if (!$ref) return;

    // Получаем slug страницы
    $slug = basename(parse_url($ref, PHP_URL_PATH));

    // Сохраняем шаблон как group_{slug}
    $template_slug = 'group_' . str_replace('-', '_', sanitize_title($slug));

    update_post_meta($post_id, 'acf_template_slug', $template_slug);

    // ✅ Логируем
    error_log("✅ Шаблон ACF сохранён: {$template_slug} для post_id {$post_id}");
}, 20);
